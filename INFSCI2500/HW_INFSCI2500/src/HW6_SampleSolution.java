...
public class HW6_SampleSolution {
    ...
    public void calculateHuffmanCodes(){
        ...
        for(int i =0;i<SIZE;i++){
            c="";
            entry = leafEntries[i];
            if(entry.freq>0){   //If the current char has freq>0, build the Huffman code.
                while(entry.parent!=null){  //Stop when we are at the root.
                    c=entry.code+c;   //Prepend the current bit to c.
                    entry=entry.parent; //Move to the current entry's parent.
                }
                leafEntries[i].code=c;  //Assign the complete Huffman code to the code field for the char entry.
            }
        }
    }
    ...
    public static void main(String[] args){
        ...
        HW6_SampleSolution h = new HW6_SampleSolution();
